...
from ..constraints.peel_pile import (                                      # noqa: F401
...
def build_combined_allocations_export(constrained_data, unconstrained_data):
    """Serialize the constrained and unconstrained tables into one .xlsx workbook.

    Returns the raw bytes of an Excel file with two sheets — "Constrained" and
    "Unconstrained" — so the user can download both halves of the allocation in a
    single file instead of two separate CSVs. Either frame may be None/empty; the
    corresponding sheet is then written empty (but still present) so the workbook
    shape is predictable. Pure (no Streamlit) so it can be unit-tested directly.
    """
    import io
    ...
    constrained = _clean(constrained_data)
    unconstrained = _clean(unconstrained_data)

    buf = io.BytesIO()
    with pd.ExcelWriter(buf, engine='openpyxl') as writer:
        # Sheet names are capped at 31 chars by Excel; ours are well under.
        constrained.to_excel(writer, sheet_name='Constrained', index=False)
        unconstrained.to_excel(writer, sheet_name='Unconstrained', index=False)
    buf.seek(0)
    return buf.getvalue()
...
def show_detailed_analysis_table(final_filtered_data, unconstrained_data, constrained_data,
    ...
    # Expose the computed allocation(s) to the Carrier Flip Analysis section so it
    # can build the flip report from the in-app optimized scenario without the user
    # re-exporting and re-uploading the file. Written every run so it tracks the
    # currently selected strategy and any filter/constraint changes.
    # The constrained frame still carries the raw carrier column (e.g.
    # 'Dray SCAC(FL)'); rename it to 'NEW SCAC' so the flip engine recognizes it as
    # the assigned carrier, matching how the constrained table labels it.
    flip_constrained = None
    if has_constraints and isinstance(constrained_download_data, pd.DataFrame):
        flip_constrained = constrained_download_data
        if carrier_col in flip_constrained.columns and 'NEW SCAC' not in flip_constrained.columns:
            flip_constrained = flip_constrained.rename(columns={carrier_col: 'NEW SCAC'})
    st.session_state['flip_source_strategy'] = selected
    st.session_state['flip_source_unconstrained'] = download_data
    st.session_state['flip_source_constrained'] = flip_constrained
    ...
    if has_constraints and constrained_download_data is not None:
        ...
        # Combined export: both tables in one multi-sheet Excel workbook.
        combined_xlsx = build_combined_allocations_export(
            constrained_download_data, download_data,
        )
        st.download_button(
            label="📥 Download Both (Constrained + Unconstrained)",
            data=combined_xlsx,
            file_name=f"allocations_combined_{selected.lower().replace(' ', '_')}.xlsx",
            mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
            use_container_width=True, key='download_combined',
            help="One Excel file with a Constrained sheet and an Unconstrained sheet.",
        )

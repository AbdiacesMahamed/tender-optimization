...
def show_optimization_settings():
    """Display optimization parameter sliders"""
    ...
    ⚠️ **Note**: Changes will not take effect until you click the "Run Optimization" button below.
    """)
    ...
    # Initialize session state for PENDING settings (slider values before "Run Optimization")
    if 'pending_cost_weight' not in st.session_state:
        st.session_state.pending_cost_weight = st.session_state.opt_cost_weight
    if 'pending_max_growth_pct' not in st.session_state:
        st.session_state.pending_max_growth_pct = st.session_state.opt_max_growth_pct
    
    st.markdown("#### ⚖️ Optimization Weights")
    st.markdown("*How much should the optimization prioritize cost vs performance?*")
    
    # Cost Weight Slider - uses pending value
    cost_weight = st.slider(
        "💰 Cost Priority (%)",
        min_value=0,
        max_value=100,
        value=st.session_state.pending_cost_weight,
        step=5,
    ...
    # Max Growth Percentage Slider - uses pending value
    max_growth = st.slider(
        "🔒 Maximum Growth Cap (%)",
        min_value=0,
        max_value=100,
        value=st.session_state.pending_max_growth_pct,
        step=5,
    ...
    # Update PENDING session state (not the active optimization settings yet)
    st.session_state.pending_cost_weight = cost_weight
    st.session_state.pending_max_growth_pct = max_growth
    
    # Check if there are pending changes
    has_pending_changes = (
        st.session_state.pending_cost_weight != st.session_state.opt_cost_weight or
        st.session_state.pending_max_growth_pct != st.session_state.opt_max_growth_pct
    )
    ...
    if has_pending_changes:
        ...
        st.warning("⚠️ You have unsaved changes. Click 'Run Optimization' to apply them.")
        
        col1, col2, col3 = st.columns(3)
        with col1:
            if st.session_state.pending_cost_weight != st.session_state.opt_cost_weight:
                st.info(f"Cost: {st.session_state.opt_cost_weight}% → **{st.session_state.pending_cost_weight}%**")
        with col2:
            if (100 - st.session_state.pending_cost_weight) != st.session_state.opt_performance_weight:
                st.info(f"Perf: {st.session_state.opt_performance_weight}% → **{100 - st.session_state.pending_cost_weight}%**")
        with col3:
            if st.session_state.pending_max_growth_pct != st.session_state.opt_max_growth_pct:
                st.info(f"Growth: {st.session_state.opt_max_growth_pct}% → **{st.session_state.pending_max_growth_pct}%**")
    
    # Run Optimization Button (primary action)
    st.markdown("---")
    col1, col2 = st.columns([2, 1])
    
    with col1:
        if st.button("▶️ Run Optimization", use_container_width=True, type="primary", disabled=not has_pending_changes):
            # Apply pending changes to active optimization settings
            st.session_state.opt_cost_weight = st.session_state.pending_cost_weight
            st.session_state.opt_performance_weight = 100 - st.session_state.pending_cost_weight
            st.session_state.opt_max_growth_pct = st.session_state.pending_max_growth_pct
    ...
    with col2:
        ...
        if st.button("🔄 Reset to Defaults", use_container_width=True):
            ...
            st.session_state.opt_performance_weight = 30
            st.session_state.opt_max_growth_pct = 30
            st.session_state.pending_cost_weight = 70
            st.session_state.pending_max_growth_pct = 30
            st.rerun()

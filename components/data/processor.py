...
# ── Inland-rail terminal → Discharged Port override ──
# National-Inland rail-ramp containers carry the BNSF ramp name in the Terminal column
# but a coastal Discharged Port (where the vessel discharged), which makes them invisible
# to inland Port-keyed constraints/rates. Rewrite Discharged Port unconditionally based
# on the terminal so these containers group with the inland port pool from the start.
TERMINAL_PORT_OVERRIDES = {
    "BNSF-LPC": "CHI",
    "BNSF-MKC": "MKC",
    "BNSF-MEM": "MEM",
}
...
def validate_and_process_gvt_data(GVTdata):
    ...
    # Override Discharged Port for inland-rail terminals (BNSF-LPC → CHI, etc.) so the
    # rest of the pipeline groups these containers with their inland port pool.
    if 'Terminal' in GVTdata.columns:
        terminal_str = GVTdata['Terminal'].astype(str).str.strip()
        override_match = terminal_str.map(TERMINAL_PORT_OVERRIDES)
        rewrite_mask = override_match.notna()
        if rewrite_mask.any():
            GVTdata.loc[rewrite_mask, 'Discharged Port'] = override_match[rewrite_mask]
...
def perform_lane_analysis(Ratedata):
    ...
    if len(duplicate_lanes) > 0:
        logger.info(f"Found {len(duplicate_lanes)} lanes with multiple rates")
...
def apply_volume_weighted_performance(merged_data):
    ...
    if 'Performance_Score' not in merged_data.columns:
        logger.info("No Performance_Score column found in merged data - skipping performance calculations")
        return merged_data
    ...
    if missing_before == total_records:
        logger.warning("All performance scores are NULL/NaN — performance data merge may have failed")
        return merged_data
    
    # If no missing scores, nothing to do
    if missing_before == 0:
        return merged_data
...
def process_performance_data(Performancedata, has_performance):
    ...
    try:
        ...
        if not week_columns:
            logger.info("No week columns (WK27, WK28, etc.) found in performance data.")
            return None, False
        ...
        if len(performance_filled) > 0:
            ...
            logger.info(f"Performance data processed: {len(performance_filled)} records from {len(week_columns)} weeks")
            if filled_count > 0:
                logger.info(f"Filled {filled_count} missing performance scores using volume-weighted averages")
            
            return performance_filled, True
        else:
            logger.warning("No valid performance data after processing")
            return None, False
            
    except Exception as e:
        logger.warning(f"Error processing performance data: {e}")
        return None, False
